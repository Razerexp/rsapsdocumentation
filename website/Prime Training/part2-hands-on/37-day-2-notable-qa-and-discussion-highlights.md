...
# Day 2 Notable Q&A and Discussion Highlights


## Certificates and Cryptography

- Q: Is the DH parameters block only in the AMS cert? A: No — it appears in every application's cert file (MS, SSP, HTAP), though not in the key or chain files. Copy it forward on every replacement.
...
- Q: How do you restart services in the field? A: systemctl (tomcat-ms/ssp/htap start|stop) on modern kits; older customers without systemctl units use the service command form instead — recognize both.
...
- Q: So what was yesterday's session token discussion about? A: Different system — the RSA authentication/session token is AMS-only (REST admin operations); SSP/HTAP ride ordinary JSESSIONID browser cookies. Redis now backs both patterns in containerized deployments.
