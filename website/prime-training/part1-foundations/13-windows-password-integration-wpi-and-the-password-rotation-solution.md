...
- Automated rotation. The Bulk Update tool ([Section 15](./15-part-2-deep-dive-the-bulk-update-tool.md)) rotates the AD password for every member of a configured group on schedule *and simultaneously updates the WPI cache in AM*. AD and AM never diverge; the user never needs to know the password at all.
