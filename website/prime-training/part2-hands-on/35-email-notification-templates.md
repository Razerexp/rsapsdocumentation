...
## The Demo and the Day's Last Lesson

The live demonstration — putting a token into lost mode to trigger its notification — was foiled by the lab itself: the MailHog container was down, so the send failed (and an earlier self-inflicted casualty surfaced: the trainer had auto-generated his own AD password that morning via the [Section 29](./29-ssp-on-demand-password-generation-completing-the-wpi-story.md) feature and no longer knew it). Prior test notifications in the MailHog inbox served as the exhibit instead, showing the workflow-generated emails with their customizable logos and text. A fitting close for a day whose deepest lesson was that things break, logs tell you why, and the fix is always one of a small number of files.
