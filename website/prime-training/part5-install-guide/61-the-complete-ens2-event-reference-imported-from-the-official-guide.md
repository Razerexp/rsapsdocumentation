...
# The Complete ENS2 Event Reference (Imported from the Official Guide)

Nineteen of the guide's eighty pages document ENS2 — by far the most complete treatment in existence, superseding both the one-paragraph training mention ([Section 3.3](../part1-foundations/3-the-rsa-prime-solution-suite.md)) and the deck procedure ([Section 47.3](../part3-deck-companion/47-deck-day-5--amis-api-ens-troubleshooting--logging-slides-6296.md)). This chapter imports it as working reference. ENS2 is a command-line Java executable run on a schedule to automate end-user and administrator notifications for token lifecycle events, and — where configured — to take action on tokens and accounts that exceed inactivity limits.

## Event Catalog and Architecture
...
## Installation Essentials (Confirming and Completing [Section 47.3](../part3-deck-companion/47-deck-day-5--amis-api-ens-troubleshooting--logging-slides-6296.md))

The guide's 32-step procedure matches [Part III](../part3-deck-companion/42-the-training-deck-overview-and-how-to-use-part-iii.md) slides 87–89 and adds the missing precision: the read-only DB account default is ensreadonly and *must match the userID in ens2-grants.sql if changed*; the rsautil create command takes an explicit netmask parameter (-n 255.255.255.255) alongside the AMIS IP; success is verified by cat'ing the ens-grants-out CSV and seeing GRANT rows; the amisdebugvar for the ENS properties is a fresh UUID (uuidgenerator.net — the same pattern as [Part IV](../part4-amis-deep-dive/50-the-amis-master-deck-provenance-and-how-part-iv-works.md) slide 30); the completed Template Builder file is saved with a password; the log directory is set to /opt/rsa/primekit/logs/ens2; execution is ../java/latest/bin/java -jar Main.jar -W 7 from the ens2 directory; and production operation is a crontab entry with test mode disabled only after validation. The file inventory: ens2-config.xml (events, intervals, template references), ens2.properties (connectivity — encryptable via Template Builder), logconfig.xml (log4j, with optional syslog appenders), Main.jar; plus, in config/: ens2-configuration.apt, ens2-grants.sql, grants.sh. Template placement rule: the bundled mail templates ship in the standard AMIS package, and any new or customized template must be copied to /opt/rsa/primekit/configs/amis/workflow/mail/templates/email — tying ENS directly into [Section 35](../part2-hands-on/35-email-notification-templates.md)'s template system.

## The Window Semantics (-W)
...
## Global Elements: testMode, send-to, whitelist
...
## Testing and Execution Recommendations (Verbatim Policy)

The guide's operating policy, adoptable as-is in engagement handoffs: test each desired event one at a time under testMode with a sane maxResults; only after every chosen interval is validated should production rollout be considered; run frequency no less than every 7 days and no more than every 30 — a weekly rhythm is most appropriate; schedule via crontab (or equivalent service automation); configure logging through logconfig.xml's standard log4j facilities, optionally with syslog appenders (the SIEM path of [Part IV](../part4-amis-deep-dive/50-the-amis-master-deck-provenance-and-how-part-iv-works.md) slide 24); and send administrative reports to an alias while keeping solution ownership with a small senior team. This chapter plus [Section 47.3](../part3-deck-companion/47-deck-day-5--amis-api-ens-troubleshooting--logging-slides-6296.md) now constitutes complete ENS coverage — from database grants to event tuning.
