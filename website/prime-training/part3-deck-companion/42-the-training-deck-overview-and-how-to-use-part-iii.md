...
The deck uses the formal component names: **AMIS** (AM Integration Services) for what the live sessions called MS/AMS, and **HDAP** (Help Desk Admin Portal) for what the transcripts rendered as HTAP. They are the same components; this edition standardizes on HDAP throughout, and the transcripts' original HTAP spelling is preserved only here for the record. Likewise the deck's service tomcat-amis command form matches the 'older customers' pattern noted in Day 2 Q\&A (Section 37.2) alongside the modern systemctl units.
