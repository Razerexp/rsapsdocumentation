...
The canonical naming: per application, `<app>`\_cert.pem, `<app>`\_key.pem, `<app>`\_chain.pem (amis/ssp/hdap), all under /opt/rsa/primekit/certificates — matching [Section 26.1](../part2-hands-on/26-certificate-management-and-replacement.md). Deck-only addition: the slide documents what truststore.jks contains — the AM root, the CAS root, and the LDAP root (the latter for AMIS-to-LDAP direct connections, [Section 29.5](../part2-hands-on/29-ssp-on-demand-password-generation-completing-the-wpi-story.md)'s bypass path). That inventory answers a recurring field question: which roots must be re-imported when any of those three back ends rotates its CA.

*Slide 41 — amis\_cert.pem and the DH Parameters*

![](/img/42.jpeg)

The formal statement of [Section 26.2](../part2-hands-on/26-certificate-management-and-replacement.md)'s rule, with an ordering detail the live session omitted: within the cert file, the CERTIFICATE block must come before the DH PARAMETERS block (BEGIN/END CERTIFICATE first, then BEGIN/END DH PARAMETERS). A replacement that pastes the DH block above the certificate fails — add this ordering check to the runbook and R-9's mitigation.

*Slide 42 — amis\_chain.pem — Chain Ordering*

![](/img/43.jpeg)

The chain rule with its clearest visual: leaf to root, ascending — the intermediate that issued the server cert first, further intermediates next, root CA last — and the explicit warning do not include the server certificate itself in the chain file (a mistake the live session's [Section 26.3](../part2-hands-on/26-certificate-management-and-replacement.md) didn't call out). 
...
The deck's formal statement of the reference architecture: at least two internal Prime nodes plus two external SSP servers, scaled per requirement, fronted by F5 / Citrix NetScaler / HAProxy — matching [Section 9.1](../part1-foundations/9-reference-production-architecture-and-sizing.md) exactly (down to the load-balancer brand list).
