---
title: "Deck Day 5 — AMIS API, ENS, Troubleshooting & Logging (Slides 62–96)"
sidebar_position: 47
---

# Deck Day 5 — AMIS API, ENS, Troubleshooting & Logging (Slides 62–96)
...
## The AMIS API, Formally (Slides 63–69)
...
The formal statement of the API model demonstrated in Sections 7.3 and 11: authentication is an HTTP POST to /auth/authn; each authentication type (RBA, token, SSO, questions…) carries a distinct POST body; multiple authentication profiles are supported with a default when unspecified ([Section 10.3](../part1-foundations/10-configuration-philosophy-file-structure-and-ms-deep-dive.md).2's profiles). Deck-only precision — the header names: session-token calls carry RSA\_AUTHENTICATION\_TOKEN:`<value>`; service-account token calls carry RSA\_SERVICE\_TOKEN=`<svctoken>`. The live Postman demos used these headers implicitly; the deck names them for anyone building an integration from scratch.

*Slide 65 — Username and Passcode Example*

![](/img/66.jpeg)

The worked request/response pair behind [Section 7.3](../part1-foundations/7-service-startup-ports-and-api-validation.md)'s demo. The XML response anatomy worth memorizing: authenticated (true/false), authenticationToken (the 8-digit session token — [Section 8.2](../part1-foundations/8-auth-clustering-and-session-token-architecture.md)'s machine-ID-encoded value), code/failed/message (ACCESS\_OK on success), and publicID.
...
The service-account request (type="serviceAccount" against profile sspauth) and its response — showing the newServiceAccountPassword tag in the wild: the rotating password arriving in the response body, exactly as taught in [Section 11.2](../part1-foundations/11-the-service-account-system-three-paths-to-a-session-token.md) and security-debated in [Section 11.4](../part1-foundations/11-the-service-account-system-three-paths-to-a-session-token.md). This slide is the client developer's contract: watch for this tag, persist the value, use it next time.

*Slide 68 — Service Account Example, Continued*

![](/img/69.jpeg)

Completes the lifecycle narrative: initial admin-set password → system-generated replacement in the response → policy block visible in the designated custom identity attribute in the Security Console ([Section 11.2](../part1-foundations/11-the-service-account-system-three-paths-to-a-session-token.md).2's encrypted expiry attribute, seen from the console side).
...
Sets a fixed passcode for emergency access with a duration in minutes — the API behind both HDAP's emergency-access operation (slide 15) and VTS's code generation ([Section 32.1](../part2-hands-on/32-virtual-token-service-vts--end-to-end-build.md)). The lostMode parameter controls concurrent token behavior: tokenauthdisablesea (token authentication allowed at any time and disables the emergency code), denytokenauth (token authentication refused for the emergency code's duration), and tokenonlyafterexpire (token authentication only after the emergency code expires). Choosing among these is a real security-design decision: deny-token is the strict-custody option; disable-on-token-use is the user-friendly one.

*Slide 85 — Move Token to Security Domain*

![](/img/86.jpeg)

Relocates a token between security domains — the API form of the domain organization Quick Setup establishes ([Section 6.4](../part1-foundations/6-installation-walkthrough-tarball-scripts-and-quick-setup.md)).

## ENS2 — Expiry Notification Service Configuration (Slides 86–89)

Deck-only procedure. ENS was introduced in one sentence on Day 1 ([Section 3.3](../part1-foundations/3-the-rsa-prime-solution-suite.md)); the deck supplies the full installation, which is nontrivial because ENS reads the AM database directly through a purpose-created read-only account:
...
## Logging, Versions, and the Formal Patch Procedure (Slides 90–96)
